...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        return Math.pow(1 + rate/100, numYears(targetYear)) * presentValue;
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        double compounded = 0;
        int i = 0;
        while (i < numYears(targetYear)) {
            compounded = (perYear + compounded)*(1 + rate/100);
            i += 1;}
        return compounded + perYear;
    }
